...
export default function Users() {
    const herokuURL = "https://cinema-webserver.herokuapp.com"
    ...
    const navigate = useNavigate()
        ...
        useEffect(async() => {
            ...
            const userDataRaw = await axios.get(`${herokuURL}/users/userfile`)
            const userData = userDataRaw.data.sort()
        
            await Promise.all(userData.map(async (user,i)=>{
                const dataraw = await axios.get(`${herokuURL}/users/${user.id}`)
                const userDataPermRaw = await axios.get(`${herokuURL}/users/permissionsfile/${user.id}`)
                let userPer;
            ...
            const id = e.target.value
            const userDBresDelete = await axios.delete(`${herokuURL}//users/${id}`)
            console.warn(userDBresDelete)
            const userJSONresDelete = await axios.delete(`${herokuURL}//users/userfile/${id}`)
            console.warn(userJSONresDelete)
            const userJSON2resDelete = await axios.delete(`${herokuURL}//users/permissionsfile/${id}`)
