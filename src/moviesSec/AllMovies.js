...
export default function AllMovies() {
  ...
  const herokuURL = "https://subs-webserver.herokuapp.com"
  useEffect(async() => {
  ...
  useEffect(async () => {
    ...
    const dataSubMoiveRaw = await axios.get(`${herokuURL}/subs`)
    const dataMoviesRaw = await axios.get(`${herokuURL}/movies`)
    console.log("movies: ")
    ...
    const subData = await Promise.all(dataSubMoiveRaw.data.map( async(sub,i)=>{
      console.log(sub)
        nameTemp = await axios.get(`${herokuURL}/members/${sub.memberId}`)
  ...
  const handleDelete =async (e)=>{
    ...
    const dataMoviesAfterDelete = await axios.delete(`${herokuURL}/movies/${e.target.value}`)
    const dataMoviesRaw = await axios.get(`${herokuURL}/movies`)
    console.log("moviesAftet delete: ")
    console.log(dataMoviesRaw.data)
    serMoviseList({moviesList:dataMoviesRaw.data})
    ////now need to handle the subs that see the movie.
    const subsDataRaw = await axios.get(`${herokuURL}/subs`)
    const newSubs = Promise.all(subsDataRaw.map(async(memberSub,i)=>{
        if(memberSub.memberId===e.target.value)
        {
          const subResDelete = await axios.get(`${herokuURL}/subs/${memberSub._id}`)
        }
        else
          return memberSub
    }))
